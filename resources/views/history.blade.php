...
<head>
    ...
    <title>Mon Historique - ZTVPlus</title>
...
<body class="bg-black min-h-screen font-sans">
    ...
    <main class="pt-24 md:pt-28 pb-16 px-4 md:px-6">
        <div class="container mx-auto">
            ...
            <div class="mb-8 md:mb-12">
                <h1 class="text-3xl md:text-5xl font-bold text-halloween-green mb-3 md:mb-4 flex items-center">
                    <i class="fas fa-history mr-3 md:mr-4"></i>
                    Mon Historique
                </h1>
            ...
            @if($historyItems->isEmpty())
                ...
                <div class="text-center py-16 md:py-24">
                    <i class="fas fa-history text-6xl md:text-8xl text-halloween-green/30 mb-6"></i>
                    <h2 class="text-2xl md:text-3xl font-bold text-text-primary mb-4">Votre historique est vide</h2>
                    <p class="text-text-secondary mb-8 max-w-md mx-auto">
                        Commencez à regarder des films et séries pour voir votre historique ici
                    </p>
                    <a href="{{ route('home') }}" class="inline-flex items-center bg-gradient-to-r from-halloween-green to-halloween-green-light text-text-primary px-6 md:px-8 py-3 md:py-4 rounded-full font-bold text-base md:text-lg shadow-2xl hover:shadow-halloween-green/50 transform hover:scale-105 transition-all duration-300">
            ...
            @else
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 md:mb-12">
                    <div class="bg-bg-secondary border-2 border-halloween-green/30 rounded-xl p-4 md:p-6">
                        <div class="text-halloween-green text-2xl md:text-3xl font-bold mb-1">{{ $totalItems }}</div>
                    ...
                    <div class="bg-bg-secondary border-2 border-halloween-yellow/30 rounded-xl p-4 md:p-6">
                        <div class="text-halloween-yellow text-2xl md:text-3xl font-bold mb-1">{{ $completedCount }}</div>
                        <div class="text-text-secondary text-xs md:text-sm">Terminés</div>
                    </div>
                </div>
                ...
                @php
                    ...
                    $groupedItems = $historyItems->groupBy(function($item) use ($userTimezone) {
                        return $item->watched_at->setTimezone($userTimezone)->format('Y-m-d');
                    });
                @endphp
                ...
                @foreach($groupedItems as $date => $items)
                    <div class="mb-8 md:mb-12">
                        ...
                        <div class="flex items-center mb-4 md:mb-6">
                            <div class="bg-gradient-to-r from-halloween-green to-halloween-green-light rounded-xl px-4 md:px-6 py-3 md:py-4">
                            ...
                            <div class="ml-4 bg-bg-secondary border-2 border-halloween-green/30 rounded-xl px-3 py-2">
                                <span class="text-halloween-green font-bold text-sm">{{ $items->count() }} élément{{ $items->count() > 1 ? 's' : '' }}</span>
                            </div>
                        </div>
                            ...
                            @foreach($items as $item)
                                ...
                                <div class="group" data-id="{{ $content->id }}" data-type="{{ $contentType }}">
                                    ...
                                    <div class="content-card">
                                        @if($isMovie)
                                        <a href="{{ route('watch.movie', ['movieId' => $content->id, 'videoId' => $item->video_id]) }}">
                                            <img src="{{ $content->poster_url }}" alt="{{ $content->title }}" class="content-poster">
                                        </a>
                                        @else
                                        <a href="{{ route('watch.episode', [
                                            'seriesId' => $content->id,
                                            'seasonNumber' => $item->video->episode->season->season_number,
                                            'episodeNumber' => $item->video->episode->episode_number,
                                        ...
                                        <!-- Remove Button -->
                                        <button onclick="removeFromHistory(event, {{ $item->id }})" class="remove-btn" title="Retirer de l'historique">
                                    ...
                                    <div class="content-info">
                                        ...
                                        <div class="flex items-center justify-between mb-2">
                                            ...
                                            <div class="text-right">
                                                <div class="text-text-secondary text-xs">
                                                    ...
                                                    {{ $item->watched_at->setTimezone($user->timezone ?? 'Europe/Paris')->format('H:i') }}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="content-details">
                                            ...
                                            @else
                                            <div class="bg-halloween-yellow/20 text-halloween-yellow px-3 py-2 rounded-lg font-semibold text-xs flex items-center justify-center">
    ...
    <!-- Footer -->
    <footer class="bg-black border-t-2 border-halloween-green py-8 md:py-12">
        ...
        function removeFromHistory(event, historyId) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showToast('info', 'Retiré de l\'historique', data.message || 'Le contenu a été retiré de votre historique');
                } else {
